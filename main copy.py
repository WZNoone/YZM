...
from NetWork.CRNN.CRNN import CRNN, Train, Test
from torch.optim.lr_scheduler import StepLR
from dataset.MyDataset_D import get_train_test
...
train_path = r'D:\S\yzm\Mark\train_all46'

test_path = r'D:\S\yzm\Mark\test_all46'


if __name__ == '__main__':
    # 加载数据集
    train, test = get_train_test(train_path, test_path, LABEL_MAP)
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = CRNN(charset_len = LABEL_MAP)
    model.to(DEVICE)

    # model.load_state_dict(torch.load('pth\\6ori0.9321.pth'))
    
    
    optimizer = torch.optim.Adam(model.parameters(), lr=0.00081, weight_decay=1e-5)
    ...
    for epoch in range(0, 200):
        ...
        for x, label, label_len in train_bar:
            x, label = x.to(DEVICE), label.to(DEVICE)
            loss, lr = Train(model, x, label, label_len, optimizer)
        ...
        # 测试循环
        for x, label, label_len in test_bar:
            x, label = x.to(DEVICE), label.to(DEVICE)
            loss, acc = Test(model, x, label, label_len)
        ...
        # 保存整体准确率
        torch.save(model.state_dict(), "pth/4ori%.4f.pth" % average_acc)
